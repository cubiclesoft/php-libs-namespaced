...
?><?php
	...
	if (!class_exists("\\CubicleSoft\\WinPEFile", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/win_pe_file.php";

	class WinPEUtils
	{
		public static function GetIconOrCursorResource($winpe, $searchtype, $icoidname = true, $icoidlang = true)
		{
			if ($searchtype !== \CubicleSoft\WinPEFile::RT_GROUP_CURSOR && $searchtype !== \CubicleSoft\WinPEFile::RT_GROUP_ICON)  return array("success" => false, "error" => "Invalid search type specified.  Must be RT_GROUP_CURSOR or RT_GROUP_ICON.", "errorcode" => "invalid_search_type");

			if (!class_exists("\\CubicleSoft\\WinICO", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/win_ico.php";
			...
			$result = \CubicleSoft\WinICO::ParseHeader($groupentry["data"], true);
			$result["group_num"] = $groupnum;
			if (!$result["success"])  return $result;
			if (($searchtype === \CubicleSoft\WinPEFile::RT_GROUP_CURSOR && $result["type"] !== \CubicleSoft\WinICO::TYPE_CUR) || ($searchtype === \CubicleSoft\WinPEFile::RT_GROUP_ICON && $result["type"] !== \CubicleSoft\WinICO::TYPE_ICO))  return array("success" => false, "error" => "Group resource type does not match.", "errorcode" => "invalid_group_resource", "group_num" => $groupnum);

			// Locate each resource and reconstruct the original icon/cursor.
			foreach ($result["icons"] as $num => $icon)
			{
				$result2 = $winpe->FindResource(($searchtype === \CubicleSoft\WinPEFile::RT_GROUP_CURSOR ? \CubicleSoft\WinPEFile::RT_CURSOR : \CubicleSoft\WinPEFile::RT_ICON), $icon["id"], $groupentry["id"]);
			...
			$result2 = \CubicleSoft\WinICO::Generate($result["type"], $result["icons"]);
		...
		{
			return self::GetIconOrCursorResource($winpe, \CubicleSoft\WinPEFile::RT_GROUP_ICON, $icoidname, $icoidlang);
		}

		public static function GetCursorResource($winpe, $icoidname = true, $icoidlang = true)
		{
			return self::GetCursorResource($winpe, \CubicleSoft\WinPEFile::RT_GROUP_CURSOR, $icoidname, $icoidlang);
		}
		...
		public static function SetIconOrCursorResource($winpe, &$data, $icoinfo, $icoidname = true, $icoidlang = true)
		{
			...
			{
				$result = \CubicleSoft\WinICO::Parse($icoinfo);
				if (!$result["success"])  return $result;
			...
			$searchtype = ($icoinfo["type"] === \CubicleSoft\WinICO::TYPE_CUR ? \CubicleSoft\WinPEFile::RT_GROUP_CURSOR : \CubicleSoft\WinPEFile::RT_GROUP_ICON);
			...
			if ($result === false)
			{
				...
				$result = \CubicleSoft\WinICO::Generate($icoinfo["type"], array());
				if (!$result["success"])  return $result;
				...
				$result = $winpe->FindResource($searchtype, $icoidname, $icoidlang);
				if ($result === false)  return array("success" => false, "error" => "Creating a group " . ($searchtype === \CubicleSoft\WinPEFile::RT_GROUP_CURSOR ? "cursor" : "icon") . " failed.", "errorcode" => "failed_creating_group_" . ($searchtype === \CubicleSoft\WinPEFile::RT_GROUP_CURSOR ? "cursor" : "icon"));
			}
			...
			foreach ($icoinfo["icons"] as $num => $icon)
			{
				if (!isset($icon["id"]))
				{
					$num2 = $winpe->CreateResourceLangNode(($searchtype === \CubicleSoft\WinPEFile::RT_GROUP_CURSOR ? \CubicleSoft\WinPEFile::RT_CURSOR : \CubicleSoft\WinPEFile::RT_ICON), true, $groupentry["id"], $icon["data"]);
			...
			$result2 = \CubicleSoft\WinICO::Generate($icoinfo["type"], $icoinfo["icons"]);
			if (!$result2["success"])  return $result2;
		...
		public static function GetUnicodeStr(&$data, &$x, $y)
		{
			...
			return \CubicleSoft\UTFUtils::Convert($str, \CubicleSoft\UTFUtils::UTF16_LE, \CubicleSoft\UTFUtils::UTF8);
		}

		public static function SetUnicodeStr(&$data, &$x, $str)
		{
			$str = \CubicleSoft\UTFUtils::Convert($str, \CubicleSoft\UTFUtils::UTF8, \CubicleSoft\UTFUtils::UTF16_LE);

			\CubicleSoft\WinPEFile::SetBytes($data, $x, $str, strlen($str) + 2);
		}
		...
		public static function Internal_ParseVersionInfoEntry(&$data, &$x, $y, $parentkey, $allowedkeys)
		{
			...
			$len = \CubicleSoft\WinPEFile::GetUInt16($data, $x, $y);
			if ($x + $len - 2 < $y)  $y = $x + $len - 2;

			$vallen = \CubicleSoft\WinPEFile::GetUInt16($data, $x, $y);
			$type = \CubicleSoft\WinPEFile::GetUInt16($data, $x, $y);
			$key = self::GetUnicodeStr($data, $x, $y);
			$x = \CubicleSoft\WinPEFile::AlignValue($x, 4);
			...
			if ($vallen)
			{
				...
				if ($parentkey === false && $key === "VS_VERSION_INFO")
				{
					...
					$entry["fixed"] = array(
						"signature" => \CubicleSoft\WinPEFile::GetUInt32($data, $x2, $y2),
						"struct_ver" => implode(".", array_reverse(array(\CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2), \CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2)))),
						"file_ver" => implode(".", array_reverse(array(\CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2), \CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2)))) . "." . implode(".", array_reverse(array(\CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2), \CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2)))),
						"product_ver" => implode(".", array_reverse(array(\CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2), \CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2)))) . "." . implode(".", array_reverse(array(\CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2), \CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2)))),
						"flags" => (\CubicleSoft\WinPEFile::GetUInt32($data, $x2, $y2) & \CubicleSoft\WinPEFile::GetUInt32($data, $x2, $y2)),
						"os" => \CubicleSoft\WinPEFile::GetUInt32($data, $x2, $y2),
						"type" => \CubicleSoft\WinPEFile::GetUInt32($data, $x2, $y2),
						"subtype" => \CubicleSoft\WinPEFile::GetUInt32($data, $x2, $y2),
						"date" => \CubicleSoft\WinPEFile::GetUInt32($data, $x2, $y2) * 0x100000000 + \CubicleSoft\WinPEFile::GetUInt32($data, $x2, $y2),
					);
				}
				...
				else if ($parentkey === "VarFileInfo" && $key === "Translation")
				{
					...
					{
						$entry["translations"][] = array(
							"lang_id" => \CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2),
							"code_page" => \CubicleSoft\WinPEFile::GetUInt16($data, $x2, $y2)
						);
					}
				}
		...
		{
			if (!class_exists("\\CubicleSoft\\UTFUtils", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/utf_utils.php";
		...
		public static function GetVersionResource($winpe, $veridlang = true)
		{
			...
			$result = $winpe->FindResource(\CubicleSoft\WinPEFile::RT_VERSION, 1, $veridlang);
		...
		public static function GenerateVersionInfoData($verinfo)
		{
			...
			$data = "";
			$x = 0;
			\CubicleSoft\WinPEFile::SetUInt16($data, $x, 0);
			\CubicleSoft\WinPEFile::SetUInt16($data, $x, (isset($verinfo["entry"]["fixed"]) ? 52 : 0));
			\CubicleSoft\WinPEFile::SetUInt16($data, $x, (isset($verinfo["entry"]["type"]) ? $verinfo["entry"]["type"] : 0));
			self::SetUnicodeStr($data, $x, "VS_VERSION_INFO");
			if ($x !== \CubicleSoft\WinPEFile::AlignValue($x, 4))  \CubicleSoft\WinPEFile::SetUInt16($data, $x, 0);

			// Fixed file information.
			if (isset($verinfo["entry"]["fixed"]))
			{
				// Signature and structure version are hardcoded.
				\CubicleSoft\WinPEFile::SetUInt32($data, $x, (int)0xFEEF04BD);
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, 0);
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, 1);

				$ver = explode(".", $verinfo["entry"]["fixed"]["file_ver"]);
				while (count($ver) < 4)  $ver[] = 0;
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, (int)$ver[1]);
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, (int)$ver[0]);
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, (int)$ver[3]);
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, (int)$ver[2]);
				...
				\CubicleSoft\WinPEFile::SetUInt32($data, $x, \CubicleSoft\WinPEFile::VERINFO_VS_FFI_FILEFLAGSMASK);
				\CubicleSoft\WinPEFile::SetUInt32($data, $x, \CubicleSoft\WinPEFile::VERINFO_VS_FFI_FILEFLAGSMASK & $verinfo["entry"]["fixed"]["flags"]);
				\CubicleSoft\WinPEFile::SetUInt32($data, $x, $verinfo["entry"]["fixed"]["os"]);
				\CubicleSoft\WinPEFile::SetUInt32($data, $x, $verinfo["entry"]["fixed"]["type"]);
				\CubicleSoft\WinPEFile::SetUInt32($data, $x, $verinfo["entry"]["fixed"]["subtype"]);

				$date = "";
				$dx = 0;
				\CubicleSoft\WinPEFile::SetUInt64($date, $dx, $verinfo["entry"]["fixed"]["date"]);
				\CubicleSoft\WinPEFile::SetBytes($data, $x, substr($date, 4), 4);
				\CubicleSoft\WinPEFile::SetBytes($data, $x, substr($date, 0, 4), 4);
			}
			...
			if (isset($verinfo["entry"]["string_file_info"]) && isset($verinfo["entry"]["string_file_info"]["string_tables"]))
			{
				...
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, 0);
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, 0);
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, (isset($verinfo["entry"]["string_file_info"]["type"]) ? $verinfo["entry"]["string_file_info"]["type"] : 1));
				...
				foreach ($verinfo["entry"]["string_file_info"]["string_tables"] as $key => $stringmap)
				{
					...
					foreach ($stringmap as $key2 => $val)
					{
						...
						\CubicleSoft\WinPEFile::SetUInt16($data, $x4, $x - $x4);
						\CubicleSoft\WinPEFile::SetUInt16($data, $x4, ($x - $x5) / 2);
					}

					\CubicleSoft\WinPEFile::SetUInt16($data, $x3, $x - $x3);
				}

				\CubicleSoft\WinPEFile::SetUInt16($data, $x2, $x - $x2);
			...
			if (isset($verinfo["entry"]["var_file_info"]))
			{
				...
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, 0);
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, 0);
				\CubicleSoft\WinPEFile::SetUInt16($data, $x, (isset($verinfo["entry"]["var_file_info"]["type"]) ? $verinfo["entry"]["var_file_info"]["type"] : 1));
				...
				if (isset($verinfo["entry"]["var_file_info"]["translations"]))
				{
					...
					foreach ($verinfo["entry"]["var_file_info"]["translations"] as $translation)
					{
						...
						\CubicleSoft\WinPEFile::SetUInt16($data, $x, $translation["lang_id"]);
						\CubicleSoft\WinPEFile::SetUInt16($data, $x, $translation["code_page"]);
					}

					\CubicleSoft\WinPEFile::SetUInt16($data, $x3, $x - $x3);
					\CubicleSoft\WinPEFile::SetUInt16($data, $x3, $x - $x4);
				}
			...
			$x2 = 0;
			\CubicleSoft\WinPEFile::SetUInt16($data, $x2, $x);
		...
		public static function SetVersionResource($winpe, &$data, $verinfo, $veridlang = true)
		{
			...
			if ($result === false)
			{
				...
				$vernum = $winpe->CreateResourceLangNode(\CubicleSoft\WinPEFile::RT_VERSION, 1, $veridlang, $verdata);
		...
		public static function CalculatePEImportsDirectoryOffsets($winpe, &$direntries)
		{
			$diroffset = 0;
			$namesize = 0;
			$bits64 = ($winpe->pe_opt_header["signature"] === \CubicleSoft\WinPEFile::OPT_HEADER_SIGNATURE_PE32_PLUS);

			foreach ($direntries as &$direntry)
			{
				$diroffset += (count($direntry["imports"]) + 1) * ($bits64 ? 8 : 4);

				$namesize += \CubicleSoft\WinPEFile::AlignValue(strlen($direntry["name"]) + 1, 2);

				foreach ($direntry["imports"] as $import)
				{
					if ($import["type"] === "named" && $import["rva"] === false)  $namesize += \CubicleSoft\WinPEFile::AlignValue(2 + strlen($import["name"]) + 1, 2);
				}
			}
			...
			return array("diroffset" => $diroffset, "nametableoffset" => $nametableoffset, "namesoffset" => $namesoffset, "namesize" => $namesize, "size" => \CubicleSoft\WinPEFile::AlignValue($namesoffset + $namesize, 4));
		}
		...
		public static function SavePEImportsDirectory($winpe, &$data, $secnum, $baserva, &$direntries)
		{
			...
			if ($secnum === false)
			{
				...
				$secnum = $winpe->GetLastPESectionIfAtEnd($data, \CubicleSoft\WinPEFile::IMAGE_SCN_CNT_INITIALIZED_DATA | \CubicleSoft\WinPEFile::IMAGE_SCN_MEM_READ | (int)\CubicleSoft\WinPEFile::IMAGE_SCN_MEM_WRITE);
				if ($secnum === false)
				{
					$result = $winpe->CreateNewPESection($data, ".idata", 0, \CubicleSoft\WinPEFile::IMAGE_SCN_CNT_INITIALIZED_DATA | \CubicleSoft\WinPEFile::IMAGE_SCN_MEM_READ | (int)\CubicleSoft\WinPEFile::IMAGE_SCN_MEM_WRITE);
					if (!$result["success"])  return $result;
			...
			if (\CubicleSoft\WinPEFile::AlignValue($baserva, 4) !== $baserva)  return array("success" => false, "error" => "Import directory base RVA is not DWORD aligned.", "errorcode" => "invalid_baserva");
			...
			foreach ($direntries as &$direntry)
			{
				...
				$size = \CubicleSoft\WinPEFile::AlignValue(strlen($direntry["name"]) + 1, 2);
				\CubicleSoft\WinPEFile::SetBytes($data, $namespos, $direntry["name"], $size);
				$namesrva += $size;

				\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $direntry["import_names_rva"]);
				\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $direntry["created"]);
				\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $direntry["forward_chain"]);
				\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $direntry["name_rva"]);
				\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $direntry["iat_rva"]);
				...
				foreach ($direntry["imports"] as &$import)
				{
					if ($import["type"] === "ord")
					{
						\CubicleSoft\WinPEFile::SetUInt16($data, $nametablepos, $import["ord"]);

						if ($bits64)  \CubicleSoft\WinPEFile::SetBytes($data, $nametablepos, "\x00\x00\x00\x00\x00\x80", 6);
						else  \CubicleSoft\WinPEFile::SetBytes($data, $nametablepos, "\x00\x80", 2);
					}
					else if ($import["type"] === "named")
					{
						if ($import["rva"] === false)
						{
							...
							\CubicleSoft\WinPEFile::SetUInt16($data, $namespos, $import["hint"]);

							$size = \CubicleSoft\WinPEFile::AlignValue(strlen($import["name"]) + 1, 2);
							\CubicleSoft\WinPEFile::SetBytes($data, $namespos, $import["name"], $size);
							$namesrva += $size + 2;
						}

						\CubicleSoft\WinPEFile::SetUInt32($data, $nametablepos, $import["rva"]);

						if ($bits64)  \CubicleSoft\WinPEFile::SetBytes($data, $nametablepos, "", 4);
					}
					else
					{
				...
				if ($bits64)
				{
					\CubicleSoft\WinPEFile::SetBytes($data, $nametablepos, "", 8);
					$nametablerva += 8;
					$iatrva += 8;
				}
				else
				{
					\CubicleSoft\WinPEFile::SetBytes($data, $nametablepos, "", 4);
					$nametablerva += 4;
					$iatrva += 4;
				}
			}

			\CubicleSoft\WinPEFile::SetBytes($data, $dirpos, "", 20);
			...
			\CubicleSoft\WinPEFile::SetBytes($data, $iatpos, $iatdata, $size);
		...
		public static function SavePEExportsDirectory($winpe, &$data, $secnum, $baserva, &$exportdir, &$addresses, &$namemap)
		{
			...
			{
				// Check the last section in the file if it has import directory-compatible flags OR create a new section.
				$secnum = $winpe->GetLastPESectionIfAtEnd($data, \CubicleSoft\WinPEFile::IMAGE_SCN_CNT_INITIALIZED_DATA | \CubicleSoft\WinPEFile::IMAGE_SCN_MEM_READ);
				if ($secnum === false)
				{
					$result = $winpe->CreateNewPESection($data, ".edata", 0, \CubicleSoft\WinPEFile::IMAGE_SCN_CNT_INITIALIZED_DATA | \CubicleSoft\WinPEFile::IMAGE_SCN_MEM_READ);
					if (!$result["success"])  return $result;
			...
			$rva = $winpe->pe_sections[$secnum]["rva"];
			if ($baserva === false)  $baserva = $rva;
			if (\CubicleSoft\WinPEFile::AlignValue($baserva, 4) !== $baserva)  return array("success" => false, "error" => "Export directory base RVA is not DWORD aligned.", "errorcode" => "invalid_baserva");
			...
			\CubicleSoft\WinPEFile::SetBytes($data, $namespos, $exportdir["name"], $size);
			$namesrva += $size;
			...
			\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $exportdir["flags"]);
			\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $exportdir["created"]);
			\CubicleSoft\WinPEFile::SetUInt16($data, $dirpos, $exportdir["major_ver"]);
			\CubicleSoft\WinPEFile::SetUInt16($data, $dirpos, $exportdir["minor_ver"]);
			\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $exportdir["name_rva"]);
			\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $exportdir["ordinal_base"]);
			\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $exportdir["num_addresses"]);
			\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $exportdir["num_name_ptrs"]);
			\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $exportdir["addresses_rva"]);
			\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $exportdir["name_ptr_rva"]);
			\CubicleSoft\WinPEFile::SetUInt32($data, $dirpos, $exportdir["ordinal_map_rva"]);

			foreach ($addresses as $addr)
			{
				if ($addr["type"] === "export")  \CubicleSoft\WinPEFile::SetUInt32($data, $addresspos, $addr["rva"]);
				else if ($addr["type"] === "forward")
				{
					\CubicleSoft\WinPEFile::SetUInt32($data, $addresspos, $namesrva);

					$size = strlen($addr["name"]) + 1;
					\CubicleSoft\WinPEFile::SetBytes($data, $namespos, $addr["name"], $size);
					$namesrva += $size;
				}
				else
				{
			...
			foreach ($namemap as $name => $ord)
			{
				\CubicleSoft\WinPEFile::SetUInt32($data, $nameptrpos, $namesrva);

				$size = strlen($name) + 1;
				\CubicleSoft\WinPEFile::SetBytes($data, $namespos, $name, $size);
				$namesrva += $size;

				\CubicleSoft\WinPEFile::SetUInt16($data, $ordinalpos, $ord);
			}
			...
			return array("success" => true, "size" => \CubicleSoft\WinPEFile::AlignValue($info["size"], 4));
		}
		...
		public static function CalculatePEBaseRelocationsDirectorySize(&$blocks)
		{
			...
			foreach ($blocks as &$block)
			{
				...
				foreach ($block["offsets"] as $offset)
				{
					...
					if ($offset["type"] === \CubicleSoft\WinPEFile::IMAGE_REL_BASED_HIGHADJ)  $size += 2;
				}

				$size = \CubicleSoft\WinPEFile::AlignValue($size, 4);
			}
		...
		public static function SavePEBaseRelocationsDirectory($winpe, &$data, $secnum, $baserva, &$blocks)
		{
			...
			$rva = $winpe->pe_sections[$secnum]["rva"];
			if ($baserva === false)  $baserva = $rva;
			if (\CubicleSoft\WinPEFile::AlignValue($baserva, 4) !== $baserva)  return array("success" => false, "error" => "Base relocations directory base RVA is not DWORD aligned.", "errorcode" => "invalid_baserva");
			...
			foreach ($blocks as &$block)
			{
				...
				\CubicleSoft\WinPEFile::SetUInt32($data, $tablepos, $block["rva"]);
				\CubicleSoft\WinPEFile::SetUInt32($data, $tablepos, $size + 8);

				foreach ($block["offsets"] as $offset)
				{
					\CubicleSoft\WinPEFile::SetUInt16($data, $tablepos, (($offset["type"] << 12) | ($offset["offset"] & 0x0FFF)) & 0xFFFF);
					$size -= 2;

					if ($offset["type"] === \CubicleSoft\WinPEFile::IMAGE_REL_BASED_HIGHADJ)
					{
						\CubicleSoft\WinPEFile::SetUInt16($data, $tablepos, $offset["extra"] & 0xFFFF);

						$size -= 2;
					}
				}

				if ($size)  \CubicleSoft\WinPEFile::SetBytes($data, $tablepos, "", $size);
			}
		...
		public static function CreateHookDLL($origfilename, $hookfilename, $winpeorig, $winpehooks, $win9x = false)
		{
			...
			if (!class_exists("\\CubicleSoft\\CSPRNG", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/random.php";
			...
			$winpe = new \CubicleSoft\WinPEFile();
			$winpe->InitPE($winpeorig->pe_opt_header["signature"]);

			$bits64 = ($winpe->pe_opt_header["signature"] === \CubicleSoft\WinPEFile::OPT_HEADER_SIGNATURE_PE32_PLUS);

			$winpe->pe_header["machine_type"] = $winpeorig->pe_header["machine_type"];
			$winpe->pe_header["flags"] = \CubicleSoft\WinPEFile::IMAGE_FILE_DLL | \CubicleSoft\WinPEFile::IMAGE_FILE_EXECUTABLE_IMAGE | ($bits64 ? \CubicleSoft\WinPEFile::IMAGE_FILE_LARGE_ADDRESS_AWARE : \CubicleSoft\WinPEFile::IMAGE_FILE_32BIT_MACHINE);

			$winpe->pe_opt_header["major_linker_ver"] = 1;
			$winpe->pe_opt_header["image_base"] = \CubicleSoft\WinPEFile::IMAGE_BASE_DLL_DEFAULT + ($rng->GetInt(0, 0x5FFF) << 16);
			...
			$winpe->pe_opt_header["dll_characteristics"] = \CubicleSoft\WinPEFile::IMAGE_DLL_CHARACTERISTICS_NO_SEH | \CubicleSoft\WinPEFile::IMAGE_DLL_CHARACTERISTICS_DYNAMIC_BASE;
			...
			// Create the section.
			$result = $winpe->CreateNewPESection($data, ".text", 0, \CubicleSoft\WinPEFile::IMAGE_SCN_CNT_CODE | \CubicleSoft\WinPEFile::IMAGE_SCN_MEM_EXECUTE | \CubicleSoft\WinPEFile::IMAGE_SCN_MEM_READ);
			if (!$result["success"])  return $result;
			...
			if ($winpeorig->pe_header["machine_type"] === \CubicleSoft\WinPEFile::IMAGE_FILE_MACHINE_AMD64)
			{
				// Expand the section.
				...
				// mov eax, 1
				// ret
				\CubicleSoft\WinPEFile::SetBytes($data, $nextpos, "\xB8\x01\x00\x00\x00\xC3\x00\x00", 8);
				$nextrva += 8;
				$bytesleft -= 8;

				$win9x = false;
			}
			else if ($winpeorig->pe_header["machine_type"] === \CubicleSoft\WinPEFile::IMAGE_FILE_MACHINE_I386)
			{
				// Expand the section.
				$result = $winpe->ExpandLastPESection($data, 8);
				$bytesleft = $result["size"];
				...
				\CubicleSoft\WinPEFile::SetBytes($data, $nextpos, "\x33\xC0\x40\xC2\x0C\x00\x00\x00", 8);
				$nextrva += 8;
				$bytesleft -= 8;
			}
			else
				...
				for ($x = 0; $x < $y; $x++)
				{
					...
					foreach ($addresses as $num => &$info)
					{
						if (!strncmp($info["name"], $prefix, $y2))
						{
							...
							\CubicleSoft\WinPEFile::SetBytes($data, $nextpos, "\xFF\x25", 2);
							\CubicleSoft\WinPEFile::SetUInt32($data, $nextpos, $winpe->pe_opt_header["image_base"] + $iatrva);
							\CubicleSoft\WinPEFile::SetBytes($data, $nextpos, "\x00\x00", 2);

							// Relocation table entry.  Points at the jmp instruction address so the DLL can be relocated.
							$relocs["offsets"][] = array("type" => \CubicleSoft\WinPEFile::IMAGE_REL_BASED_HIGHLOW, "offset" => (($nextrva + 2) & 0x0FFF));
			...
			{
				// Create the section.
				$result = $winpe->CreateNewPESection($data, ".reloc", 0, \CubicleSoft\WinPEFile::IMAGE_SCN_CNT_INITIALIZED_DATA | \CubicleSoft\WinPEFile::IMAGE_SCN_MEM_DISCARDABLE | \CubicleSoft\WinPEFile::IMAGE_SCN_MEM_READ);
				if (!$result["success"])  return $result;
