	...
	class IFDS_Text
	{
		...
		protected function ReadDataInternal($obj)
		{
			...
			if ($obj->GetEncoder() === self::ENCODER_DEFLATE)
			{
				...
				if (!\CubicleSoft\DeflateStream::IsSupported())  return array("success" => false, "error" => self::IFDSTextTranslate("Unable to decompress data.  Deflate/zlib stream filter support is not enabled."), "errorcode" => "zlib_support_not_enabled");

				$result["data"] = \CubicleSoft\DeflateStream::Uncompress($result["data"]);
		...
		protected function WriteDataInternal($obj, &$data)
		{
			...
			{
				if (class_exists("\\CubicleSoft\\DeflateStream", false) || file_exists(str_replace("\\", "/", dirname(__FILE__)) . "/deflate_stream.php"))
				{
					require_once str_replace("\\", "/", dirname(__FILE__)) . "/deflate_stream.php";

					if (\CubicleSoft\DeflateStream::IsSupported())
					{
						$data2 = \CubicleSoft\DeflateStream::Compress($data, $this->compresslevel);
						if ($data2 !== false)
						{
							$y2 = strlen($data2);
			...
			if ($obj->GetEncoder() !== \CubicleSoft\IFDS::ENCODER_RAW)  $result = $this->ifds->SetObjectEncoder($obj, \CubicleSoft\IFDS::ENCODER_RAW);
			...
			do
			{
				...
				if ($size > 65527 || $last)
				{
					...
					if ($nextwrite === $nextwrite2 || ($writesuper === $readsuper && $nextwrite === $nextread))
					{
						...
						$result = $this->ifds->CreateRawData(\CubicleSoft\IFDS::ENCODER_RAW);
						if (!$result["success"])  return $result;
