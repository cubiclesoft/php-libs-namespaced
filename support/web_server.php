...
?><?php
	...
	class WebServer
	{
		...
		public function Start($host, $port, $sslopts = false)
		{
			...
			if (is_array($sslopts))
			{
				stream_context_set_option($context, "ssl", "ciphers", \CubicleSoft\HTTP::GetSSLCiphers());
		...
		public function Wait($timeout = false, $readfps = array(), $writefps = array(), $exceptfps = NULL)
		{
			...
			foreach ($this->clients as $id => $client)
			{
				if ($client->lastts + $this->defaultclienttimeout < $ts)
				{
					if ($client->requests)
					{
						$result2 = array("success" => false, "error" => \CubicleSoft\HTTP::HTTPTranslate("Client timed out.  Most likely cause:  Connection failure."), "errorcode" => "client_timeout");
