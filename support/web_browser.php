...
?><?php
	...
	class WebBrowser
	{
		...
		{
			if (!class_exists("\\CubicleSoft\\HTTP", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/http.php";
				...
				switch ($state["state"])
				{
					case "initialize":
					{
						...
						if (substr($profile, 0, 2) == "ie" || ($profile == "auto" && substr($this->data["useragent"], 0, 2) == "ie"))
						{
							...
							$headers["User-Agent"] = \CubicleSoft\HTTP::GetUserAgent(substr($profile, 0, 2) == "ie" ? $profile : $this->data["useragent"]);
						}
						else if ($profile == "firefox" || ($profile == "auto" && $this->data["useragent"] == "firefox"))
						{
							...
							$headers["User-Agent"] = \CubicleSoft\HTTP::GetUserAgent("firefox");
						}
						else if ($profile == "opera" || ($profile == "auto" && $this->data["useragent"] == "opera"))
						{
							...
							$headers["User-Agent"] = \CubicleSoft\HTTP::GetUserAgent("opera");
						}
						else if ($profile == "safari" || $profile == "chrome" || ($profile == "auto" && ($this->data["useragent"] == "safari" || $this->data["useragent"] == "chrome")))
						{
							...
							$headers["User-Agent"] = \CubicleSoft\HTTP::GetUserAgent($profile == "safari" || $profile == "chrome" ? $profile : $this->data["useragent"]);
						}
						...
						if ($state["timeout"] !== false)  $state["options"]["timeout"] = \CubicleSoft\HTTP::GetTimeLeft($state["startts"], $state["timeout"]);
						...
						// Process the request.
						$result = \CubicleSoft\HTTP::RetrieveWebpage($state["url"], $state["options"]);
					...
					case "process_async":
					{
						...
						$result = \CubicleSoft\HTTP::ProcessState($state["httpstate"]);
					...
					case "post_retrieval":
					{
						...
						if (isset($state["result"]["headers"]["Location"]) && $this->data["followlocation"])
						{
							...
							if ($this->data["referer"] != "")  $state["url"] = \CubicleSoft\HTTP::ConvertRelativeToAbsoluteURL($this->data["referer"], $state["url"]);

							$urlinfo2 = \CubicleSoft\HTTP::ExtractURL($state["url"]);
						...
						if (isset($state["result"]["headers"]["Set-Cookie"]))
						{
							foreach ($state["result"]["headers"]["Set-Cookie"] as $cookie)
							{
								...
								if ($item != "")
								{
									...
									{
										$ts = \CubicleSoft\HTTP::GetDateTimestamp($cookie["expires"]);
						...
						else
						{
							...
							// Call \CubicleSoft\WebBrowser::GenerateFormRequest() to prepare an actual request for Process().
		...
		public function Process($url, $profile = "auto", $tempoptions = array())
		{
			...
			if ($this->data["referer"] != "")  $url = \CubicleSoft\HTTP::ConvertRelativeToAbsoluteURL($this->data["referer"], $url);
		...
		public function ProcessAsync__Handler($mode, &$data, $key, &$info)
		{
			switch ($mode)
			{
				...
				case "readfps":
				{
					if ($info["state"]["httpstate"] !== false && \CubicleSoft\HTTP::WantRead($info["state"]["httpstate"]))  $data[$key] = $info["state"]["httpstate"]["fp"];

					break;
				}
				case "writefps":
				{
					if ($info["state"]["httpstate"] !== false && \CubicleSoft\HTTP::WantWrite($info["state"]["httpstate"]))  $data[$key] = $info["state"]["httpstate"]["fp"];
				...
				case "cleanup":
				{
					...
					if ($data === true)
					{
						if (isset($info["state"]))
						{
							if ($info["state"]["httpstate"] !== false)  \CubicleSoft\HTTP::ForceClose($info["state"]["httpstate"]);
		...
		public function ExtractForms($baseurl, $data, $hint = false)
		{
			...
			if ($this->html === false)
			{
				if (!class_exists("\\CubicleSoft\\simple_html_dom", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/simple_html_dom.php";

				$this->html = new \CubicleSoft\simple_html_dom();
			}
			$this->html->load($data);
			...
			foreach ($rows as $row)
			{
				...
				$info["action"] = (isset($row->action) ? \CubicleSoft\HTTP::ConvertRelativeToAbsoluteURL($baseurl, (string)$row->action) : $baseurl);
				...
				$form = new \CubicleSoft\WebBrowserForm();
				$form->info = $info;
				$form->fields = $fields;
				$result[] = $form;
			}
			...
			if (count($form->GetVisibleFields(false)))
			{
				...
				do
				{
					...
					foreach ($form->fields as $num => $field)
					{
						...
						echo \CubicleSoft\HTTP::HTTPTranslate("\t%d:  %s - %s\n", $num + 1, $field["name"], (is_array($field["value"]) ? json_encode($field["value"], JSON_PRETTY_PRINT) : $field["value"]) . (($field["type"] == "input.radio" || $field["type"] == "input.checkbox") ? ($field["checked"] ? \CubicleSoft\HTTP::HTTPTranslate(" [Y]") : \CubicleSoft\HTTP::HTTPTranslate(" [N]")) : "") . ($field["hint"] !== "" ? " [" . $field["hint"] . "]" : ""));
					}
					echo "\n";
	...
	class WebBrowserForm
	{
		...
		public function GenerateFormRequest($submitname = false, $submitvalue = false)
		{
			...
			if ($method == "get")
			{
				...
				$result = array(
					"url" => \CubicleSoft\HTTP::CondenseURL($url),
					"options" => array()
				);
			}
			else
