...
?><?php
	...
	class TwilioSDK
	{
		...
		public function ValidateWebhookRequest($checksig = true)
		{
			...
			if (!class_exists("\\CubicleSoft\\Str", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/str_basics.php";
			if (!class_exists("\\CubicleSoft\\Request", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/request.php";

			$valid = ((isset($_POST["AccountSid"]) && \CubicleSoft\Str::CTstrcmp($this->sid, $_POST["AccountSid"]) === 0) || (isset($_GET["AccountSid"]) && \CubicleSoft\Str::CTstrcmp($this->sid, $_GET["AccountSid"]) === 0));
			...
			$url = \CubicleSoft\Request::GetFullURLBase();
			...
			if (!isset($_SERVER["HTTP_X_TWILIO_SIGNATURE"]) || \CubicleSoft\Str::CTstrcmp(base64_encode(self::hash_hmac_internal("sha1", $url, $this->token, true)), $_SERVER["HTTP_X_TWILIO_SIGNATURE"]) !== 0)
			{
				http_response_code(403);
