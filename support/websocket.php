...
?><?php
	...
	class WebSocket
	{
		...
		public function Connect($url, $origin, $options = array(), $web = false)
		{
			...
			if (class_exists("\\CubicleSoft\\CSPRNG", false) && $this->csprng === false)  $this->csprng = new \CubicleSoft\CSPRNG();
			...
			else
			{
				...
				if ($web === false)  $web = new \CubicleSoft\WebBrowser();

				// Transform URL.
				$url2 = \CubicleSoft\HTTP::ExtractURL($url);
				if ($url2["scheme"] != "ws" && $url2["scheme"] != "wss")  return array("success" => false, "error" => self::WSTranslate("\\CubicleSoft\\\CubicleSoft\WebSocket::Connect() only supports the 'ws' and 'wss' protocols."), "errorcode" => "protocol_check");
				...
				if ($result["response"]["code"] != 101)  return array("success" => false, "error" => self::WSTranslate("\\CubicleSoft\\\CubicleSoft\WebSocket::Connect() failed to connect to the WebSocket.  Server returned:  %s %s", $result["response"]["code"], $result["response"]["meaning"]), "errorcode" => "incorrect_server_response");
