...
?><?php
	...
	class WebSocketServer
	{
		...
		{
			if (!class_exists("\\CubicleSoft\\WebSocket", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/websocket.php";

			$this->fp = false;
			$this->clients = array();
			$this->nextclientid = 1;
			$this->websocketclass = "\\CubicleSoft\\WebSocket";
			$this->origins = false;

			$this->defaultclosemode = \CubicleSoft\WebSocket::CLOSE_IMMEDIATELY;
		...
		private function ProcessInitialResponse($method, $path, $client)
		{
			...
			if ($client->writedata === "")
			{
				...
				$client->writedata .= "Sec-WebSocket-Accept: " . base64_encode(sha1($client->headers["Sec-Websocket-Key"] . \CubicleSoft\WebSocket::KEY_GUID, true)) . "\r\n";
