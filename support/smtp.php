...
?><?php
	...
	if (!class_exists("\\CubicleSoft\\IPAddr", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/ipaddr.php";
	...
	class SMTP
	{
		...
		public static function ConvertToRFC1342($data, $lang = "UTF-8", $encodeb64 = true)
		{
			...
			if ($encodeb64)
			{
				...
				if ($lang == "UTF-8")
				{
					...
					$pos = 0;
					$size = 0;
					while (\CubicleSoft\UTF8::NextChrPos($data, $y, $pos, $size))
					{
		...
		public static function MakeValidEmailAddress($email, $options = array())
		{
			...
			$local = \CubicleSoft\UTF8::MakeValid($local);
			...
			if (substr($domain, -1) == "]")
			{
				...
				if (strtolower(substr($domain, 0, 6)) == "[ipv6:")  $ipaddr = \CubicleSoft\IPAddr::NormalizeIP(substr($domain, 6, -1));
				else  $ipaddr = \CubicleSoft\IPAddr::NormalizeIP(substr($domain, 1, -1));
		...
		public static function EmailAddressesToEmailHeaders($emailaddrs, $headername, $multiple = true, $removenames = false, $options = array())
		{
			...
			for ($x = 0; $x < $y && ($multiple || $result == ""); $x++)
			{
				...
				if ($name != "" && !\CubicleSoft\UTF8::IsASCII($name))  $name = self::ConvertToRFC1342($name) . " ";
		...
		protected static function GetIDNAHost($host)
		{
			...
			if ($x < $y)
			{
				...
				$host2 = \CubicleSoft\UTFUtils::ConvertToPunycode($host);
		...
		public static function ConvertHTMLToText($data)
		{
			...
			// Load TagFilter.
			if (!class_exists("\\CubicleSoft\\TagFilter", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/tag_filter.php";

			$data = \CubicleSoft\UTF8::MakeValid($data);

			$options = \CubicleSoft\TagFilter::GetHTMLOptions();
			...
			$data = \CubicleSoft\TagFilter::Run($data, $options);
		...
		public static function CreateDeliveryStatusMessage($host, $fromaddr, $toaddr, $subject, $notificationmsg, $sender, $origrecipient, $finalrecipient, $action, $status, $diagcode, $origmsg = false, $options = array())
		{
			...
			$subject = str_replace("\n", " ", $subject);
			if (!\CubicleSoft\UTF8::IsASCII($subject))  $subject = self::ConvertToRFC1342($subject);
		...
		public static function SendEmail($fromaddr, $toaddr, $subject, $options = array())
		{
			...
			else
			{
				...
				for ($x = 0; $x < $y; $x++)
				{
					...
					$type = str_replace("\n", "", $type);
					$type = \CubicleSoft\UTF8::ConvertToASCII($type);
					...
					else
					{
						...
						$location = \CubicleSoft\UTF8::ConvertToASCII($location);
					}
					...
					else
					{
						...
						$cid = str_replace("\n", "", $cid);
						$cid = \CubicleSoft\UTF8::ConvertToASCII($cid);
					}
