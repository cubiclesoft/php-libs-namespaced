...
?><?php
	...
	class IFDS_Conf
	{
		...
		public function Create($pfcfilename, $options = array())
		{
			...
			if (!class_exists("\\CubicleSoft\\IFDS", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/ifds.php";

			$this->ifds = new \CubicleSoft\IFDS();
	...
	class IFDS_ConfDef
	{
		...
		public function GetContext($name)
		{
			...
			if ($obj->GetEncoder() !== \CubicleSoft\IFDS::ENCODER_KEY_ID_MAP)  return array("success" => false, "error" => self::IFDSConfTranslate("Context is not a key-ID map."), "errorcode" => "invalid_data_method");
		...
		{
			$result = $this->ifds->CreateLinkedListNode(\CubicleSoft\IFDS::ENCODER_RAW);
			if (!$result["success"])  return $result;
		...
		public function UpdateOption($obj, $type, $options = array())
		{
			...
			\CubicleSoft\IFDS_Conf::AppendTypeData($data, $type, $options["defaults"]);
		...
		public static function ExtractOptionData(&$data)
		{
			...
			$options["defaults"] = \CubicleSoft\IFDS_Conf::ExtractTypeData($data, 12 + $size, $options["type"], $multiple);
		...
		public function GetOption($id)
		{
			...
			if ($obj->GetEncoder() !== \CubicleSoft\IFDS::ENCODER_RAW)  return array("success" => false, "error" => self::IFDSConfTranslate("Option is not raw data."), "errorcode" => "invalid_data_method");
		...
		public function GetOptionValues($id)
		{
			...
			if ($obj->GetEncoder() !== \CubicleSoft\IFDS::ENCODER_KEY_ID_MAP)  return array("success" => false, "error" => self::IFDSConfTranslate("Option values are not a key-ID map."), "errorcode" => "invalid_data_method");
		...
		{
			$result = $this->ifds->CreateLinkedListNode(\CubicleSoft\IFDS::ENCODER_KEY_VALUE_MAP);
			if (!$result["success"])  return $result;
		...
		public function GetDoc($id)
		{
			...
			if ($obj->GetEncoder() !== \CubicleSoft\IFDS::ENCODER_KEY_VALUE_MAP)  return array("success" => false, "error" => self::IFDSConfTranslate("Documentation object is not a key-value map."), "errorcode" => "invalid_data_method");
