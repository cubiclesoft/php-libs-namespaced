...
?><?php
	...
	class ZipStreamWriter
	{
		...
		public static function IsDeflateSupported()
		{
			...
			return \CubicleSoft\DeflateStream::IsSupported();
		}
		...
		{
			if (!class_exists("\\CubicleSoft\\CRC32Stream", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/crc32_stream.php";
			if (!class_exists("\\CubicleSoft\\DeflateStream", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/deflate_stream.php";
		...
		public function AddFileFromString($filename, $data, $options = array(), $compresslevel = -1)
		{
			...
			$deflatesupported = (isset($options["compress_method"]) && $options["compress_method"] === self::COMPRESS_METHOD_STORE ? false : \CubicleSoft\DeflateStream::IsSupported());

			// Compress the data in advance.  If the compressed version ends up larger than the original, just store it instead.
			if ($deflatesupported)
			{
				$data2 = \CubicleSoft\DeflateStream::Compress($data, $compresslevel);
		...
		public function OpenFile($filename, $options = array(), $compresslevel = -1)
		{
			...
			// Set up data streams.
			$this->currcrc32 = new \CubicleSoft\CRC32Stream();
			$this->currcrc32->Init();

			if ($deflatesupported)
			{
				$this->currds = new \CubicleSoft\DeflateStream();
		...
		{
			$cancompress = \CubicleSoft\DeflateStream::IsSupported();
			...
			$extrafields[] = array(
				...
				"var" => ($cancompress ? \CubicleSoft\DeflateStream::Compress($os2extendedattrs) : $os2extendedattrs)
			);
		}
