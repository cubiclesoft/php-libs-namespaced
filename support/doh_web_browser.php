...
?><?php
	...
	class DOHWebBrowser extends \CubicleSoft\WebBrowser
	{
		...
		public function InternalDNSOverHTTPSHandler(&$state)
		{
			...
			if (strpos($host, ":") === false && !preg_match('/^[0-9.]+$/', $host))
			{
				...
				{
					if ($this->dohweb === false)  $this->dohweb = new \CubicleSoft\WebBrowser();
				...
				$state["url"] = \CubicleSoft\HTTP::CondenseURL($state["urlinfo"]);
			}
