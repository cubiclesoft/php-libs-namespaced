...
?><?php
	...
	if (!class_exists("\\CubicleSoft\\WebMutex"))  require_once "web_mutex.php";
	...
	class ReadWriteLock
	{
		...
		{
			$this->name = $name;
			$this->mutex = new \CubicleSoft\WebMutex($name);
		...
		public function Lock($write = false, $block = true, $maxlock = false)
		{
			...
			if ($write)
			{
				...
				$this->writermutex = new \CubicleSoft\WebMutex($this->name . ".writer");
