...
?><?php
	...
	// Load dependencies.
	if (!class_exists("\\CubicleSoft\\WebBrowser", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/web_browser.php";
	...
	class DigitalOcean
	{
		...
		{
			if (!class_exists("\\CubicleSoft\\CLI", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/cli.php";
			...
			do
			{
				...
				$url = \CubicleSoft\CLI::GetUserInputWithArgs($args, false, self::DO_Translate("Authorization URL"), false, self::DO_Translate("Once you sign in, your web browser will be redirected to an invalid URL that starts with '%s'.  That will be the Authorization URL to enter for the next question.", $tokens["returnurl"]));
				if (substr($url, 0, strlen($tokens["returnurl"])) !== $tokens["returnurl"])  \CubicleSoft\CLI::DisplayError("The URL supplied is not valid.", false, false);
				else
				{
					$url = \CubicleSoft\HTTP::ExtractURL($url);

					if (isset($url["queryvars"]["error"]))  \CubicleSoft\CLI::DisplayError(self::DO_Translate("Unfortunately, an error occurred:  %s (%s).  Did you deny/cancel the consent?", $url["queryvars"]["error_description"][0], $url["queryvars"]["error"][0]), false, false);
					else
					{
						$result = $this->UpdateRefreshToken($url["queryvars"]["code"][0]);
						if (!$result["success"])  \CubicleSoft\CLI::DisplayError(self::DO_Translate("Unfortunately, an error occurred while attempting to retrieve the refresh/bearer tokens:  %s (%s).  Try the process again.", $result["error"], $result["errorcode"]), false, false);
						else  $valid = true;
					}
				}
			} while (!$valid);
