...
?><?php
	...
	if (!class_exists("\\CubicleSoft\\CSDB", false))  require_once str_replace("\\", "/", dirname(__FILE__)) . "/db.php";

	class CSDB_sqlite extends \CubicleSoft\CSDB
	{
		protected $dbprefix;
		...
		{
			return \CubicleSoft\CSDB::DB_Translate("SQLite (via PDO)");
		}
		...
		protected function GenerateSQL(&$master, &$sql, &$opts, $cmd, $queryinfo, $args, $subquery)
		{
			switch ($cmd)
			{
				...
				case "DROP COLUMN":
				{
					...
					if (!$row)  return array("success" => false, "error" => \CubicleSoft\CSDB::DB_Translate("An error occurred while getting existing table information."), "errorcode" => "show_create_table_error");

					if (!isset($row->opts[1][$queryinfo[1]]))  return array("success" => false, "errorcode" => "skip_sql_query");

					unset($row->opts[1][$queryinfo[1]]);

					if (!count($row->opts[1]))  return array("success" => false, "error" => \CubicleSoft\CSDB::DB_Translate("Table contains only one column."), "errorcode" => "drop_column_error");
		...
		protected function ProcessColumnDefinition($info)
		{
			...
			switch ($type)
			{
				...
				default:  return array("success" => false, "error" => \CubicleSoft\CSDB::DB_Translate("Unknown column type '%s'.", $type), "errorcode" => "unknown_column_type");
			}
		...
		protected function ProcessKeyDefinition($info)
		{
			...
			switch ($type)
			{
				...
				default:  return array("success" => false, "error" => \CubicleSoft\CSDB::DB_Translate("Unknown key type '%s'.", $type), "errorcode" => "unknown_key_type");;
			}
